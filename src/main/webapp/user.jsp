...
<div id="content">
        ...
        <div class="content-search">
            <c:if test="${sessionScope.authUser.role == 'ADMIN'}">
                <form action="registration.jsp">
                    <button type="submit"><i class='far fa-address-book'></i> <fmt:message key="user.button.register_user"/></button>
                </form>
            </c:if>
        </div>
    </div>
    ...
    <form action="controller" method="post">
                        ...
                        <select name="userRole">
                            <c:choose>
                                <c:when test="${user.role == 'ADMIN'}">
                                    <option value="admin" selected="selected"><fmt:message key="user.label.admin"/></option>
                                    <option value="user"><fmt:message key="user.label.user"/></option>
                                </c:when>
                                <c:otherwise>
                                    <option value="admin"><fmt:message key="user.label.admin"/><</option>
                                    <option value="user" selected="selected"><fmt:message key="user.label.user"/></option>
                                </c:otherwise>
                            </c:choose>
                        </select>
                    </td>
        ...
        <div class="content-submit-btn-main">
            ...
            <button class="content-submit-btn" type="submit" name="command" value="find_user"><fmt:message key="user.button.cancel"/></button>
        ...
        <c:if test="${authUser.role == 'ADMIN'}">
            <div class="content-submit-btn-deactivate">
                <button class="content-submit-btn content-sub-btn-d-style"
                        type="submit" name="command" value="deactivate_user"><fmt:message key="user.button.deactivate_account"/>
                </button>
            </div>
        </c:if>
    </form>
