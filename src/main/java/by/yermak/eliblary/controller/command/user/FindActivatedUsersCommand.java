...
public class FindActivatedUsersCommand implements Command {
    private static final Logger LOGGER = LogManager.getLogger();

    private final UserService userService;

    public FindActivatedUsersCommand() {
    ...
    public Router execute(HttpServletRequest request, HttpSession session) {
        ...
        if (isAuthorized(session) && isAdmin(session)) {
            try {
                List<User> userList = new ArrayList<>(userService.findActivatedUsers());
                List<User> users = new ArrayList<>(userService.findActivatedUsers(currentPage));
