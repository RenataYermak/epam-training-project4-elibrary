...
public class OrderDaoImpl implements OrderDao {
    ...
    private static class Query {
        ...
        public static final String SELECT_ORDERED_BOOKS_BY_USER_ID_AND_STATUS =
                "SELECT  orders.order_id, orders.status, orders.issue, orders.ordered_date, " +
                        "books.title, users.login,  FROM orders " +
                        "INNER JOIN books ON orders.book_id = books.book_id" +
                        "INNER JOIN users ON orders.user_id = users.user_id" +
                        "WHERE users.user_id=? AND book_orders.status=?";
        public static final String INSERT_ORDER =
                "INSERT INTO orders (user_id, book_id, status, issue, ordered_date, reserved_date, returned_date, rejected_date) " +
                        "VALUES(?, ?, ?, ?, ?, ?, ?)";
        public static final String SET_BOOKS_NUMBER_TO_ONE_LESS =
                "UPDATE books SET number=number-1 " +
                        "WHERE book_id=(SELECT book_id FROM orders WHERE order_id=?);";
        public static final String SET_BOOKS_NUMBER_TO_ONE_MORE =
                "UPDATE books SET number=number+1" +
                        " WHERE book_id=(SELECT book_id FROM orders WHERE order_id=?);";
        public static final String SELECT_ORDER_BY_ID = "SELECT order_id, book_id, user_id, status, issue," +
                "ordered_date, rejected_date, returned_date, reserved_date FROM orders WHERE order_id=?";
        public static final String SELECT_ALL_ORDERS = """
                SELECT order_id,book_id, user_id, status, issue, ordered_date,
                rejected_date, returned_date, reserved_date
                FROM orders
                """;
    }
    ...
    public List<Order> findAll() throws DaoException {
        ...
        try (Connection connection = ConnectionPool.getInstance().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(Query.SELECT_ALL_ORDERS);
    ...
    @Override
    public Optional<Order> create(Order order) throws DaoException {
        LOGGER.log(Level.INFO, "method create order");
        try (Connection connection = ConnectionPool.getInstance().getConnection();
             PreparedStatement preparedStatementOrder = connection.prepareStatement(Query.INSERT_ORDER, Statement.RETURN_GENERATED_KEYS);
             PreparedStatement preparedStatementUpdateBookNumber = connection.prepareStatement(Query.SET_BOOKS_NUMBER_TO_ONE_LESS)) {
            constructPreparedStatement(preparedStatementOrder, order);
            preparedStatementOrder.executeUpdate();
            try (ResultSet resultSet = preparedStatementOrder.getGeneratedKeys()) {
                if (resultSet.next()) {
                    order.setId(resultSet.getLong(1));
                    Optional<Order> optionalOrder = find(order.getId());
                    if (order.getId() != null) {
                        preparedStatementUpdateBookNumber.setLong(1, order.getId());
    ...
    private void constructPreparedStatement(PreparedStatement preparedStatement, Order order) throws SQLException {
        preparedStatement.setLong(1, order.getUserId());
        preparedStatement.setLong(2, order.getBookId());
        preparedStatement.setString(3, order.getIssue().toString());
        preparedStatement.setString(4, order.getStatus().toString());
        preparedStatement.setTimestamp(5, order.getOrderedDate());
        preparedStatement.setTimestamp(6, order.getReturnedDate());
        preparedStatement.setTimestamp(7, order.getRejectedDate());
        preparedStatement.setTimestamp(8, order.getReservedDate());
    }
...
//        try (Connection connection = ConnectionPool.getInstance().getConnection();
//                PreparedStatement psOrderBook = connection.prepareStatement(Query.ORDER_BOOK, Statement.RETURN_GENERATED_KEYS);
//             PreparedStatement psUpdateBookNumber = connection.prepareStatement(Query.SET_BOOKS_NUMBER_TO_ONE_LESS)) {
